	...
	namespace System
	{
		namespace Graphics
		{
			...
			void Device::InitPipeline()
			{
				...
				HRESULT result = CompileShaderFromFile( "Shaders/generic.fx", "VS", "vs_5_0", &pVSBlob );
				if( FAILED( result ) )
				{
				...
				result = CompileShaderFromFile( "Shaders/generic.fx", "PS", "ps_5_0", &pPSBlob );
				if( FAILED( result ) )
				{
			...
			void Device::InitGeometry()
			{
				...
				mpDevice->CreateBuffer( &bd, &initData, &mpVertexBuffer );
